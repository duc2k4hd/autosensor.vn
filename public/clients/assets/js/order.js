...
async function onWardChange(el) {
    ...
    if (hasSelection(value)) {
                ...
                if (
                    ...
                    try {
                        ...
                        if (json?.code === 200 && json?.data && typeof json.data === "object") {
                            ...
                            const shippingContainer = document.querySelector('.autosensor_main_checkout_options');
        ...
        document.querySelector(
            ".autosensor_main_checkout_options"
        ).innerHTML = `
              <div style="
                  padding: 16px;
...
function validateFormCheckout() {
    ...
    if (!shippingOriginalVal || shippingOriginalVal <= 0) {
        showOnce("Vui lòng chọn địa chỉ để hệ thống lấy phí vận chuyển trước khi đặt hàng.", document.querySelector('.autosensor_main_checkout_options'), false);
        isValid = false;
    }
...
function setupAddressAutocomplete() {
    const input = document.getElementById(
        "autosensor_main_checkout_form_address"
    );
    ...
    const ensureDropdown = () => {
        if (!dropdown) {
            ...
            dropdown.className = "autosensor_main_checkout_form_address_all";
        ...
        debounceTimer = setTimeout(async () => {
            ...
            try {
                ...
                box.innerHTML = items
                    .map(
                        (item) =>
                            `<p class="autosensor_main_checkout_form_address_item">${item.address.label}</p>`
                    )
                    .join("");
                ...
                box.querySelectorAll(
                    ".autosensor_main_checkout_form_address_item"
                ).forEach((el) => {
