...
@section(
    ...
    (
        ...
        ?? $post->meta_title
        ?? $post->title
    ) . ' | ' . ($settings->site_name ?? 'AutoSensor Việt Nam')
)
...
@section('content')
    ...
    <div class="autosensor_blog_social-icons">
        <a href="#" class="autosensor_blog_social-icon autosensor_blog_zalo" title="Zalo">
            <i class="fa fa-comment"></i>
        </a>
        <a href="#" class="autosensor_blog_social-icon autosensor_blog_messenger" title="Messenger">
            <i class="fab fa-facebook-messenger"></i>
        </a>
        <a href="#" class="autosensor_blog_social-icon autosensor_blog_chat" title="Chat">
            <i class="fa fa-comments"></i>
        </a>
        <a href="tel:19006026" class="autosensor_blog_social-icon autosensor_blog_phone" title="Gọi điện">
            <i class="fa fa-phone"></i>
        </a>
        <a href="#" class="autosensor_blog_social-icon autosensor_blog_location" title="Vị trí">
            <i class="fa fa-map-marker-alt"></i>
        </a>
        <a href="#" class="autosensor_blog_social-icon autosensor_blog_facebook" title="Facebook">
    ...
    <!-- Main Layout -->
    <div class="autosensor_blog_main-wrapper">
        <!-- Left Sidebar - TOC (Replaced menu with TOC) -->
        <aside class="autosensor_blog_left-sidebar" id="autosensor_blog_leftSidebar">
            <div class="autosensor-article-sidebar-content">
                @desktop
                    <!-- TOC - Replaced menu with TOC -->
                    @if($toc->isNotEmpty())
                        <div class="autosensor-article-toc" id="toc-desktop">
                            <div class="autosensor-article-toc-title">
                                <p>📑 Mục lục</p>
                            </div>
                            <ul class="autosensor-article-toc-list">
                                @foreach($toc as $item)
                                    <li class="{{ $item['tag'] === 'h3' ? 'autosensor-article-toc-item-h3' : '' }}">
        ...
        <main class="autosensor_blog_content-area">
                ...
                @if($galleryImages->isNotEmpty())
                    <div class="autosensor-article-carousel" id="postImageCarousel">
                        <div class="autosensor-article-carousel-inner">
                            @foreach($galleryImages as $index => $image)
                                @php
                                    ...
                                    $isLcp = $index === 0;
                                @endphp
                                <div class="autosensor-article-carousel-item {{ $isLcp ? 'active' : '' }}">
                                    <img src="{{ $imgUrl }}" alt="{{ $image->alt ?? $post->title }}" class="autosensor_blog_featured-image"
                                         @if($isLcp)
                                         loading="eager"
                                         fetchpriority="high"
                                         @else
                        ...
                        @if($galleryImages->count() > 1)
                            <button class="autosensor-article-carousel-prev" type="button" aria-label="Previous">
                                <span aria-hidden="true">‹</span>
                            </button>
                            <button class="autosensor-article-carousel-next" type="button" aria-label="Next">
                                <span aria-hidden="true">›</span>
                            </button>
                        @endif
                    </div>
                @else
                    <img src="{{ $firstImage }}" alt="{{ $post->title }}" class="autosensor_blog_featured-image" loading="eager" fetchpriority="high">
                @endif
                {{-- <div class="autosensor_blog_featured-overlay">
            ...
            <!-- Article Content -->
            <article class="autosensor_blog_article">
                <div class="autosensor_blog_article-header">
                    @if($post->category)
                        <span class="autosensor_blog_article-category">{{ $post->category->name }}</span>
                    @endif
                    <h1 class="autosensor_blog_article-title">{{ $post->title }}</h1>
                    <div class="autosensor_blog_article-date">
                    ...
                    @endphp
                    <!-- Author Info -->
                    <div class="autosensor-article-author-info" style="display: flex; align-items: center; gap: 12px; margin-top: 20px; padding-top: 20px; border-top: 1px solid #e5e5e5;">
                        <div class="autosensor-article-author-avatar" style="width: 48px; height: 48px; border-radius: 50%; overflow: hidden; flex-shrink: 0;">
                        ...
                        <div class="autosensor-article-author-details">
                ...
                @mobile
                    <!-- TOC Mobile -->
                    @if($toc->isNotEmpty())
                        <div class="autosensor-article-toc" id="toc-mobile" style="margin: 20px 30px; background: #f8f8f8; padding: 20px; border-radius: 8px;">
                            <div class="autosensor-article-toc-title">
                                <p>📑 Mục lục</p>
                            </div>
                            <ul class="autosensor-article-toc-list">
                ...
                <div class="autosensor_blog_article-content">
                    {!! $contentWithAnchors !!}
                </div>

                <!-- Tags Section -->
                @if($tags->isNotEmpty())
                    <div class="autosensor_blog_article-tags">
                ...
                <!-- Comments Section -->
                <div class="autosensor-article-comments" style="padding: 0 30px 40px; border-top: 1px solid #e5e5e5; margin-top: 30px; padding-top: 30px;">
                ...
                @if($relatedPosts->isNotEmpty())
                    <div class="autosensor_blog_related-posts">
                        <div class="autosensor_blog_related-posts-title">
                            <h3>📰 Bài viết liên quan</h3>
                        </div>
                        <div class="autosensor_blog_related-posts-grid">
                            @foreach($relatedPosts as $related)
                                ...
                                <a href="{{ route('client.blog.show', $related) }}" class="autosensor_blog_related-post-card">
                                    ...
                                    <div class="autosensor_blog_related-post-content">
                                        <h4 class="autosensor_blog_related-post-title">{{ str()->limit($related->title, 70) }}</h4>
                                        <span class="autosensor_blog_related-post-date">📅 {{ optional($related->published_at)->format('d/m/Y') ?? $related->created_at->format('d/m/Y') }}</span>
                                    </div>
                                </a>
                            @endforeach
                        </div>
    ...
    <!-- Support Button -->
    <div class="autosensor_blog_support-button" onclick="window.open('https://zalo.me/{{ $settings->contact_zalo ?? $settings->contact_phone ?? '' }}', '_blank')">
...
@section('foot')
    <script>
        ...
        function toggleSidebar() {
            const sidebar = document.getElementById('autosensor_blog_leftSidebar');
            if (sidebar) {
                sidebar.classList.toggle('autosensor_blog_active');
            }
        }
        ...
        document.addEventListener('click', function(event) {
            const sidebar = document.getElementById('autosensor_blog_leftSidebar');
            const toggle = document.querySelector('.autosensor_blog_menu-toggle');
            
            if (sidebar && toggle) {
                if (!sidebar.contains(event.target) && !toggle.contains(event.target)) {
                    sidebar.classList.remove('autosensor_blog_active');
                }
            }
        });
        ...
        document.querySelectorAll('.autosensor_blog_article-content img').forEach(img => {
            img.style.opacity = '0';
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            const contentSections = document.querySelectorAll('.autosensor_blog_article-content h2[id], .autosensor_blog_article-content h3[id]');
            ...
            const images = document.querySelectorAll('.autosensor_blog_article-content img:not([loading])');
            images.forEach(img => {
