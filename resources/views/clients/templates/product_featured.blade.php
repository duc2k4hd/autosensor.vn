<div class="autosensor_product_featured">
    ...
    <div class="autosensor_single_desc_tabs_describe_product_new_grid">
        @if (!empty($featuredProductCards))
            @foreach ($featuredProductCards as $featured)
                <!-- Item -->
                <div class="autosensor_single_desc_tabs_describe_product_new_item">
                    <div class="autosensor_single_desc_tabs_describe_product_new_img">
                        <a href="/{{ $featured['slug'] ?? 'san-pham-noi-bat' }}">
                            <img loading="lazy" decoding="async" src="{{ asset('clients/assets/img/clothes/resize/230x230/' . ($featured['image_url'] ?? 'no-image.webp')) }}"
                                srcset="
                                    {{ asset('clients/assets/img/clothes/' . ($featured['image_url'] ?? 'no-image.webp')) }} 1050w,
                                    {{ asset('clients/assets/img/clothes/' . ($featured['image_url'] ?? 'no-image.webp')) }} 155w
                                "
                                ...
                                alt="{{ $featured['name'] ?? '' }}">
                            <span class="autosensor_single_desc_tabs_describe_product_new_badge" style="{{ $featured['badge']['style'] ?? 'background: #ffd700;' }}">{{ $featured['badge']['label'] ?? '⭐ Hot' }}</span>
                        </a>
                    </div>
