@php
    // withQueryString() đã tự động giữ lại tất cả query parameters, không cần thêm thủ công
...
@if ($paginator->hasPages())
    <ul class="pagination">
        ...
        @else
            <li class="page-item"><a href="{{ $paginator->previousPageUrl() }}" class="page-link" rel="prev">{!! $prev_icon ?? '&laquo;' !!}</a></li>
        @endif

        {{-- Pagination Elements (hiển thị: [1] [...] [current] [...] [last]) --}}
        @php
            ...
            if ($last <= 5) {
                ...
                if ($current <= 2) {
                    ...
                    $pagesToShow = [1, 2, 3, $last];
                } elseif ($current >= $last - 1) {
                    // Trang cuối: 1, ..., last-2, last-1, last
                    $pagesToShow = [1, $last - 2, $last - 1, $last];
                } else {
                    // Trang giữa (3, 4, 5, ...): 1, ..., current, ..., last
                    $pagesToShow = [1, $current, $last];
                }
            }
        @endphp
        ...
        @foreach ($pagesToShow as $index => $page)
            @php
                $prevPage = $index > 0 ? $pagesToShow[$index - 1] : null;
                $needEllipsis = $prevPage !== null && $page - $prevPage > 1;
            @endphp
        ...
        @if ($paginator->hasMorePages())
            <li class="page-item"><a href="{{ $paginator->nextPageUrl() }}" class="page-link" rel="next">{!! $next_icon ?? '&raquo;' !!}</a></li>
        @else
